...
CREATE PROC [dbo].[GC_MESRules_Select_By_Customer_ID_DetectedStepIns_CRDStepIns] 
(
	@Customer_ID		int,
	@DetectedStepIns	nvarchar(50),
	@Defect_ID			int = 0,
	@CRD				nvarchar(27) = '',
	@CRDStepIns			nvarchar(50) -- MES Step Ins
)
AS
BEGIN
	SET NOCOUNT ON
	...
	-- from @CRDStepIns => @StepIns (Group)
	SET @StepIns = [dbo].[ufn_GC_CRDStepIns_Find_CRDStepIns](@Customer_ID, @CRDStepIns)
	--IF @StepIns = ''
	--BEGIN
	--	SET @StepIns = N'[All]'
	--END
	...
	--PRINT 'A'
		...
		WHERE [Customer_ID] = @Customer_ID
				AND [DetectedStationID] = @DetectedStationID
				AND ([CRDStepIns] = @StepIns OR [CRDStepIns] = '[All]')
				AND [Defect_ID] = @Defect_ID
				AND ([CRD] = @CRD  OR [CRD] LIKE @CRD + '/%')
				AND [IsActive] = 1
				AND [IsExcluded] = 0
	...
	--PRINT 'E'
		...
		WHERE [Customer_ID] = @Customer_ID
				...
				AND [IsExcluded] = 0
	END
	SELECT C.[StepIns] AS [EscapedStepIns]
	...
	WHERE C.[Customer_ID] = @Customer_ID
			AND A.[EscapedStationID] NOT IN (SELECT B.[EscapedStationID]
									...
									WHERE  	B.[Customer_ID] = @Customer_ID
										...
										AND (B.[CRDStepIns] = @StepIns OR B.[CRDStepIns] = '[All]')
