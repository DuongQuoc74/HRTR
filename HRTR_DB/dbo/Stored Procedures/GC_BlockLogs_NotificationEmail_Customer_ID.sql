CREATE PROC [dbo].[GC_BlockLogs_NotificationEmail_Customer_ID] 
(
	@Customer_ID			int
...
BEGIN
	...
	--	SET @EndDate = GETDATE()
	SET @StartDate = DATEADD(DAY, -@Aging, [dbo].[ufn_GC_CreatedDate](@CurrentDate))
	SET @EndDate = DATEADD(DAY, -@Aging, GETDATE())
	--SET @EndDate = DATEADD(DAY, 1, [dbo].[ufn_GC_CreatedDate](@CurrentDate))
	--SET @EndDate = DATEADD(DAY, -@Aging, @EndDate)
	--SET @StartDate = DATEADD(DAY, -1, @EndDate)
	--PRINT @StartDate
	--PRINT @EndDate
	...
	BEGIN
		...
		IF @NotificationTime = 1 -- Trainer + Trainer Leader
		BEGIN
			SET @recipientslist = [dbo].[ufn_GetEmailList_Trainer] ()
								+ N';' + [dbo].[ufn_GetEmailList_TrainerLeader](@Customer_ID)
		END
		ELSE IF @NotificationTime = 2 -- Trainer + Trainer Leader + Line Manager
		BEGIN
			SET @recipientslist = [dbo].[ufn_GetEmailList_Trainer] ()
								+ N';' + [dbo].[ufn_GetEmailList_TrainerLeader](@Customer_ID)
								+ N';' + [dbo].[ufn_GetEmailList_LineManager](@Customer_ID)
		END
		ELSE IF @NotificationTime = 3 -- Trainer + Trainer Leader + Line Manager + QE
		BEGIN
			SET @recipientslist = [dbo].[ufn_GetEmailList_Trainer] ()
								+ N';' + [dbo].[ufn_GetEmailList_TrainerLeader](@Customer_ID)
								+ N';' + [dbo].[ufn_GetEmailList_LineManager](@Customer_ID)
								+ N';' + [dbo].[ufn_GetEmailList_QE](@Customer_ID)
		END
