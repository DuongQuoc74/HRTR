CREATE PROC [dbo].[rptTrainingRecord_OrientationRecord] 
(
...
BEGIN
	...
	BEGIN
		IF @TrainingStatusID != 0
			SET @Query2 = @Query2 + N' AND (CASE WHEN C.[TrainingRecordID] IS NOT NULL THEN 1 ELSE 2 END) = @TrainingStatusID' 
	END
	IF @OutputTypeID = 1
	BEGIN
