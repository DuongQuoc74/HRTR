...
CREATE PROC [dbo].[TrainingRecord_Search_20170705] 
(
...
BEGIN
	...
	IF @CourseID != 0
		SET @Query2 = @Query2 + + N' AND H.[CourseID] = @CourseID' 
	IF @CourseIDList != ''
		SET @Query2 = @Query2 + + N' AND H.[CourseID] IN (SELECT [Items] AS [CourseID] FROM [dbo].[Split](@CourseIDList, '';''))' 
	IF @CourseGroupID != 0
