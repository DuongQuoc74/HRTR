...
-- From CCS V1.0.0.3
-- V1.0.0.5
CREATE PROC [dbo].[CCS_AuthenticationLog_Start]
(
	@UserName			varchar(50),
...
BEGIN
	...
	DECLARE @CCS_AuthenticationLogID	int = 0
			...
			-- e Grape Chart
			, @IsBlockedGC				bit = 0
			, @eGrapeChartFullPath		nvarchar(255) = ''
			, @EstimatedBlockedDateGC	datetime
			, @IsAppliedCCS				varchar(20) = ''
	...
	END
	-- Auto add new server START
	IF NOT EXISTS (SELECT 1
				FROM [dbo].[CCS_Terminal] WITH (NOLOCK)
				WHERE [TerminalName] = @TerminalName)
	BEGIN
		IF @TerminalName LIKE '%VA%' OR @TerminalName LIKE '%SHB%'
		BEGIN
			SET @IsVA = 1
			SET @MESFullPath = ''
		...
		BEGIN
			SET @IsVA = 0
			SET @MESFullPath = 'C:\Program Files (x86)\MES\MES.exe'
			SET @eGrapeChartFullPath = 'C:\Program Files (x86)\Jabil Circuit\e Grape Chart\eGrapeChartWpf.exe'
		END
    ...
    IF @EmployeeID_ID != -1 AND @ServerStatusID = 1 -- normal server
    BEGIN
	...
	SET @IsCCS = ISNULL(@IsCCS, 1) -- 1
	-- for whole system
	IF @IsCCS = 1
	BEGIN
		SELECT TOP 1 @IsAppliedCCS = CONVERT(varchar(20), [ConfigValue])
		FROM [dbo].[CCS_Config] WITH (NOLOCK)
		WHERE [ConfigName] = 'IsAppliedCCS'
		SET @IsCCS = [dbo].[ufn_ToBit] (@IsAppliedCCS)
	END
	IF @DepartmentID = 7 --IT 
	BEGIN
		SET @IsCCS = 0
	END
	-- for users who do not exists in HRHR
	IF	NOT EXISTS (SELECT 1
				FROM [dbo].[HR_Employee] WITH (NOLOCK)
				WHERE [eDBUserName] = @UserName)
