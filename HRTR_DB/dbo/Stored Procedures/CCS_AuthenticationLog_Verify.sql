...
-- From CCS V1.0.0.3
CREATE PROCEDURE [dbo].[CCS_AuthenticationLog_Verify] 
(
...
BEGIN
	...
	BEGIN
		...
		BEGIN
			SELECT TOP 1 @RequireVABeforeMES = [dbo].[ufn_ToBit](ISNULL([ConfigValue], '1'))
			FROM [dbo].[CCS_Config] WITH (NOLOCK)
			WHERE [ConfigName] = 'RequireVABeforeMES' -- 
			SET @RequireVABeforeMES = ISNULL(@RequireVABeforeMES, 1)
			IF @RequireVABeforeMES = 1
			BEGIN
